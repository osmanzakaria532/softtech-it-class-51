...
class DUP_PRO_Package
{
    ...
    public function __construct(
        ...
        $this->Name = $this->getNameFromFormat($template);
    ...
    public function create_scan_report()
    {
        ...
        try {
            ...
            $report['ARC']['Status']['Network']                = $report['ARC']['Status']['HasNotImportableSites'] ||
                $report['ARC']['Status']['HasFilteredSites'] ? 'Warn' : 'Good';
...
//DATABASE
    ...
    /**
     * Generate a package name from a template
     *
     * @param ?DUP_PRO_Package_Template_Entity $template Template to use
     *
     * @return string
     */
    protected function getNameFromFormat(DUP_PRO_Package_Template_Entity $template = null)
    {
        if ($template == null) {
            return NameFormat::getNameFromFormat(NameFormat::DEFAULT_FORMAT);
        } else {
            return NameFormat::getNameFromFormat($template->package_name_format, $this->schedule_id, $template->getId());
        }
    }
